...
const ResultsPage = () => {
  ...
  const { income, year } = router.query;
  const { loading, error, results, fetchTaxBrackets } = useTaxBrackets(income, year);
  ...
  return <Box>
      ...
      <Button type="submit" variant="contained" size="large" sx={{ mt: 2 }} onClick={() => router.push('/')}>
        Go Back
      </Button>
    </Box>;
};
